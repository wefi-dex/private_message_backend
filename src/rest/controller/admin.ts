...
interface UserRow {
  ...
  creator_approval_date: Date | null
...
export const deleteUser = async (req: Request, res: Response) => {
  try {
    ...
    const deleteQuery =
      'DELETE FROM "User" WHERE id = $1 RETURNING id, username'
...
export const banUser = async (req: Request, res: Response) => {
  try {
    ...
    const userCheckQuery =
      'SELECT id, username, banned FROM "User" WHERE id = $1'
    const userCheckResult = await pool.query(userCheckQuery, [id])
    ...
    // Ban the user
    const banQuery =
      'UPDATE "User" SET banned = TRUE, updated_at = NOW() WHERE id = $1 RETURNING id, username'
...
export const unbanUser = async (req: Request, res: Response) => {
  try {
    ...
    // Unban the user
    const unbanQuery =
      'UPDATE "User" SET banned = FALSE, updated_at = NOW() WHERE id = $1 RETURNING id, username'
...
export const getDashboardStats = async (req: Request, res: Response) => {
  try {
    ...
    // Get pending reports
    const pendingQuery =
      'SELECT COUNT(*) as pending_reports FROM "UserReport" WHERE status = \'pending\''
    ...
    // Get banned users
    const bannedQuery =
      'SELECT COUNT(*) as banned_users FROM "User" WHERE banned = TRUE'
    ...
    const stats = {
      totalUsers: parseInt(
        (usersResult.rows[0] as StatsRow).total_users || '0',
      ),
      totalReports: parseInt(
        (reportsResult.rows[0] as StatsRow).total_reports || '0',
      ),
      pendingReports: parseInt(
        (pendingResult.rows[0] as StatsRow).pending_reports || '0',
      ),
      totalBlocks: parseInt(
        (blocksResult.rows[0] as StatsRow).total_blocks || '0',
      ),
      bannedUsers: parseInt(
        (bannedResult.rows[0] as StatsRow).banned_users || '0',
      ),
...
export const approveCreator = async (req: Request, res: Response) => {
  try {
    ...
    const userCheckQuery =
      'SELECT id, username, role, creator_approved FROM "User" WHERE id = $1'
    const userCheckResult = await pool.query(userCheckQuery, [id])
